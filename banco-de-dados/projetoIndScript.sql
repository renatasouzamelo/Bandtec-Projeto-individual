...
select nomeCliente, emailCliente, historico.*
               from  cliente
                      join historico
                              on fk_cliente = id_cliente;
                              
select nomeLivro,livraria.*
               from livro
                      join livraria
                              on fk_livraria =id_livraria;
                              
                              
                              
 select fk_cliente, sum(quantidadeVendas) as vendas,
                    avg(quantidadeVendas) as media
				         from cliente
							 join historico
                                 on fk_cliente = id_cliente
								 group by quantidadeVendas;             
